...
root = "C:\\Users\\Username\\Downloads\\DeepLesion\\"
model_path = "model21_loss0.3077.pt"
# Read CSV files containing Filenames, Labels, and Suplementary Data
df = pd.read_csv(root + "broadlesion.csv")
...
class LesionDetection(datasets.VisionDataset):
    ...
    def __init__(self, root = root + "prep\\", split=[3], transform=None, target_transform=None, transforms=None):
    ...
    def _load_image(self, id):
        id = self.labels[id]
        path = root + "minideeplesion\\" + id["filename"]
...
for epoch in range(num_epochs):
    ...
    if best_loss < valid_score[-1]:
        ...
        # Save the Model
        torch.save(model.state_dict(), f"model_enhanced{epoch}_loss{valid_score[-1]}.pt")
